...
if (isset($_POST['submit'])){
    ...
    $DOB=$_POST['DOB'];
    $dateCreate=date("Y-m-d"); //Date Create
      // First name check
    ...
    //Middle name check
    if(preg_match($name_regex,$middleName)){
    ...
    //Family Name
    if(preg_match($name_regex,$familyName)){
    ...
    if(
        ...
        $sql="INSERT INTO phpform (first_name, middle_name, last_name, final_name, email, user_password, confirm_password, phone_number, date_of_birth, date_create)
            VALUES ('$firstName', '$middleName', '$lastName', '$familyName','$email', '$password', '$confirmPassword','$phoneNumber', '$DOB', '$dateCreate'); ";
        if(mysqli_query($con, $sql)){ //To write the query in the database
